function Par = CircleFitByTaubin(X, Y)

    n = length(X);  
    centroid = mean([X, Y]);  
    XY = [X, Y] - centroid;  % Centering data

    % Computing moments using vectorized operations
    Xi = XY(:, 1);
    Yi = XY(:, 2);
    Zi = Xi.^2 + Yi.^2;
    ...
    for iter = 1:IterMax
        ...
        if abs(ynew) > abs(yold), xnew = 0; break; end
...
% %--------------------------------------------------------------------------
% 
% n = max(size(X,1));      % number of data points
% 
% XY = zeros(n, 2);
% 
% XY(:, 1) = X;
